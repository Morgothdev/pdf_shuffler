...
object Runner {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val parser = new OptionParser[Config]("shuffler") {
      ...
      arg[File]("<file>...") minOccurs 1 unbounded() required() action {
        (x, c) =>
  ...
  def doWithConfig(config: Config) {
    implicit val logger = Log(config.verbose)
  ...
  def doWithFile(file: File, config: Config, logger: Log) {
    ...
    withOpenDocument(document) {
      document => {
        val pages = loadPages(document)
        logger.log("loaded " + pages.length + " pages from " + file.getName)
        val tuples = pages.zipWithIndex
        val pagesInCrop = config.pagesOnPage * config.sheetsInCrop * 4
        logger.log("in one crop will be " + pagesInCrop + " pages from pdf")
        ...
        if (!config.numbers) {
          ...
          withOpenDocument(new PDDocument()) {
            document => {
              book foreach (document importPage _._1)
              withOpenStream(new FileOutputStream(outFile)) {
                stream =>
